...
class RecordingThread(threading.Thread):
    ...
    def __init__(self, filename, duration, sample_rate=16000):
        ...
        super().__init__()
        self.filename = str(filename)  # Ensure filename is a string
    ...
    def run(self):
        ...
        try:
            ...
            if frames and not self._stop_event.is_set():
                logger.info(f"Saving {len(frames)} frames to {self.filename}")
                try:
                    # Directly use fixed parameters rather than trying to derive them
                    ...
                    self.error = f"Error saving recording: {e}"
                    raise
...
class TrainingPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, config, on_model_trained=None):
    ...
    def _refresh_files(self):
        """Refresh file lists"""
        # Log directories
        logger.debug(f"Scanning wake word directory: {self.wake_word_dir}")
        logger.debug(f"Scanning background directory: {self.negative_dir}")
    ...
    def _create_ui(self):
        ...
        instructions = ctk.CTkLabel(
            self,
            text="Train your wake word model by recording samples or adding existing WAV files to the training directories.",
            wraplength=600
        )
    ...
    def _update_recording_progress(self, mode, duration):
    ...
    def _recording_finished(self, mode):
    ...
    def _update_training_progress(self, status_text, progress_value):
