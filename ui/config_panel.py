...
logger = logging.getLogger("Io.UI.Config")

class ConfigPanel(ctk.CTkFrame):
    """Configuration panel using CustomTkinter"""
    
    def __init__(self, parent, config, audio_capture, detector, on_save_callback=None):
    ...
    def _create_ui(self):
        ...
        action_option3 = ctk.CTkRadioButton(
            action_frame, text="Keyboard Shortcut", 
            variable=self.action_type, value="keyboard",
            command=self._on_action_type_change
        )
        action_option3.pack(anchor="w", padx=10, pady=5)
        
        action_option4 = ctk.CTkRadioButton(
            action_frame, text="Custom Script", 
            variable=self.action_type, value="custom_script",
            command=self._on_action_type_change
        )
        action_option4.pack(anchor="w", padx=10, pady=5)
    ...
    def _test_audio(self):
        ...
        # Audio level bar
        self.audio_level = ctk.CTkProgressBar(level_frame, width=200)
        self.audio_level.pack(side="left", padx=10, fill="x", expand=True)
        ...
        # Start audio testing
        self._update_audio_level(device_index, dialog)
    
    def _update_audio_level(self, device_index, dialog):
        """Update audio level meter"""
        if not dialog.winfo_exists():
            return
            
        # Get audio level from recent samples (mock implementation)
        # In a real implementation, this would get data from the audio capture module
        try:
            level = self.audio_capture.get_audio_level(device_index)
            self.audio_level.set(min(1.0, level))
        except Exception as e:
            logger.error(f"Error getting audio level: {e}")
        
        # Schedule next update
        dialog.after(100, lambda: self._update_audio_level(device_index, dialog))
    ...
    def _on_action_type_change(self):
        ...
        if action_type == "notification":
            ...
            self.param_label.configure(text="Shortcut:")
    ...
    def _set_param_value(self):
        ...
        if action_type == "notification":
            ...
            param_value = action_params.get("shortcut", "")
        elif action_type == "custom_script":
    ...
    def _test_config(self):
        ...
        self.detector.register_test_callback(on_detection)
    ...
    def _close_test_dialog(self, dialog, original_threshold):
        ...
        # Unregister detector callback
        self.detector.unregister_test_callback()
    ...
    def _save_config(self):
        ...
        try:
            ...
            if model_name and model_name != "No models found":
                model_path = str(Path.home() / ".io" / "models" / model_name)
